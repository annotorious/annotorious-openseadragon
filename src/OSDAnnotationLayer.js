...
export class AnnotationLayer extends EventEmitter {
  constructor(props) {
    ...
    if (props.config.formatter) this.formatters = [props.config.formatter];
    else if (props.config.formatters)
      this.formatters = Array.isArray(props.config.formatters)
        ? props.config.formatters
        : [props.config.formatters];
    ...
    const onLoad = () => {
      ...
      if (!this.loaded) this.emit('load', src);
    ...
    if (this.viewer.world.getItemAt(0)) onLoad();
  ...
  _getShapeAt = (evt) => {
    ...
    if (annotation) return this.findShape(annotation);
  };
  ...
  _initDrawingTools = (gigapixelMode) => {
    ...
    this.mouseTracker = new OpenSeadragon.MouseTracker({
      ...
      preProcessEventHandler: (info) => {
        ...
        if (this.selectedShape && info.eventType === 'wheel') {
          ...
          this.viewer.canvas.dispatchEvent(
            new info.originalEvent.constructor(info.eventType, info.originalEvent),
          );
        }
      },
      ...
      pressHandler: (evt) => {
        if (!this.tools.current.isDrawing) {
          ...
          if (!gigapixelMode) this.scaleTool(this.tools.current);
        }
      },
      ...
      releaseHandler: (evt) => {
        if (this.tools.current.isDrawing) {
          ...
          if (dragging && this.tools.current.onDragEnd) this.tools.current.onDragEnd();
        }
    ...
    if (this.onKeyUp) document.removeEventListener('keydown', this.onKeyDown);
  ...
  _initMouseEvents = () => {
    ...
    this.viewer.addHandler('canvas-press', () => (lastMouseDown = new Date().getTime()));
  ...
  addDrawingTool = (plugin) => this.tools.registerTool(plugin);

  addOrUpdateAnnotation = (annotation, previous) => {
    const selected = this.selectedShape?.annotation;
    if (selected === annotation || selected?.isSelection || selected == previous) this.deselect();

    if (previous) this.removeAnnotation(annotation);
  ...
  _fit = (annotationOrId, opts, fn) => {
    ...
    if (shape) {
      const immediately = opts ? (typeof opts == 'boolean' ? opts : opts.immediately) : false;
  ...
  fitBounds = (annotationOrId, immediately) => this._fit(annotationOrId, immediately, 'fitBounds');
  ...
  getAnnotationsIntersecting = (annotationOrId) => {
    const annotation = annotationOrId.id
      ? annotationOrId
      : this.findShape(annotationOrId).annotation;
  ...
  getImageSnippetById = (annotationId) => {
    ...
    if (shape) return getSnippet(this.viewer, shape);
  };
  ...
  init = (annotations) => {
    ...
    this._lazy(() => {
      ...
      if (!this.loaded) this.g.style.display = 'none';
  ...
  listDrawingTools = () => this.tools.listTools();
  ...
  panTo = (annotationOrId, immediately) => {
    ...
    if (shape) {
      ...
      const center = this.viewer.viewport.windowToViewportCoordinates(
  ...
  removeAnnotation = (annotationOrId) => {
    ...
    if (this.selectedShape?.annotation.id === id) this.deselect();
    ...
    if (toRemove) {
      ...
      if (this.selectedShape?.annotation === annotation) this.deselect();
  ...
  removeDrawingTool = (id) => this.tools?.unregisterTool(id);
  ...
  resize() {
    ...
    if (this.tools?.current) this.scaleTool(this.tools.current);
  }
  ...
  scaleFormatterElements = (opt_shape) => {
    ...
    if (opt_shape) {
      ...
      if (el) el.firstChild.setAttribute('transform', `scale(${scale})`);
    } else {
      const elements = Array.from(this.g.querySelectorAll('.a9s-formatter-el'));
      elements.forEach((el) => el.firstChild.setAttribute('transform', `scale(${scale})`));
    }
  };
  ...
  scaleTool = (tool) => {
    if (tool) {
      ...
      if (tool.onScaleChanged) tool.onScaleChanged(scale);
    }
  };

  selectAnnotation = (annotationOrId, skipEvent) => {
    if (this.selectedShape) this.deselect();
    ...
    if (selected) {
      ...
      const element = this.selectedShape.element ? this.selectedShape.element : this.selectedShape;
  ...
  selectShape = (shape, skipEvent) => {
    ...
    // Don't re-select
    if (this.selectedShape?.annotation === shape.annotation) return;

    // If another shape is currently selected, deselect first
    if (this.selectedShape && this.selectedShape.annotation !== shape.annotation) this.deselect();
    ...
    if (!(readOnly || this.headless)) {
      ...
      if (toolForAnnotation) {
        setTimeout(() => {
          ...
          if (!skipEvent) this.emit('select', { annotation, element: this.selectedShape.element });
        }, 1);
        ...
        if (!skipEvent) this.emit('select', { annotation, element: this.selectedShape });
      }
    } else {
      this.selectedShape = shape;
      addClass(shape, 'selected');

      if (!skipEvent) this.emit('select', { annotation, element: shape, skipEvent });
    }
  };
  ...
  setDrawingEnabled = (enable) => {
    if (this.mouseTracker) {
      ...
      if (this.tools.current) this.tools.current.enabled = enabled;
    }
  };
  ...
  stopDrawing = () => {
    if (this.tools?.current) {
      if (this.tools.current.isDrawing) this.tools.current.stop();
